...
//  std::cout<<"this is the work type of node-1 "<<tname<<std::endl;
...
#define MXNET_USE_DIST_KVSTORE
#endif*/
#if MXNET_USE_DIST_KVSTORE
//	   LOG(INFO)<<"node-1 dist "<<tname;
...
//	   LOG(INFO)<<"node-1 kvstore!! "<<tname;
      kv = new kvstore::KVStoreDist(use_device_comm);
//      LOG(INFO)<<"node-1 kvstore creates successful!! "<<tname;
    }
    if (!has("_async") && kv->IsWorkerNode() && kv->get_rank() == 0) {
      // configure the server to be the sync mode
//      LOG(INFO)<<"node-1 is a worker node!";
