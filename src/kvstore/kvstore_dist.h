...
class KVStoreDist : public KVStoreLocal {
 public:
  explicit KVStoreDist(bool use_device_comm)
    ...
    if (IsWorkerNode()) {
//	    std::cout<<"node-1 is a worker node kvstore_dist"<<std::endl;
...
//    std::cout << "node-1 msg_size_limit = " << msg_size_limit << std::endl;
  }
 ...
 private:
  ...
  void InitImpl(const std::vector<int>& keys,
    ...
    if (get_rank() == 0 && this->ps_worker_->get_customer()->customer_id() == 0) {
	   // std::cout<<"node-1 dist InitImpl"<<std::endl;
  ...
  void PushPullImpl(const std::vector<int>& vkeys,
    ...
    //std::cout<<"node-1 PushPullImpl!"<<std::endl;
  ...
  void PushImpl(const std::vector<int>& keys,
                const std::vector<NDArray>& values,
                int priority) override {
	  //std::cout<<"node-1 PushImpl!"<<std::endl;
  ...
  void PullImpl(const std::vector<int>& keys,
	  ...
	  //std::cout<<"node-1 PullImpl!"<<std::endl;
    ...
    //std::cout<<"Node-1 uniq_keys.size() "<<uniq_keys.size()<<std::endl;
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      int key = uniq_keys[i];
//      std::cout<<"Node-1 keys  "<<key<<std::endl;
  ...
  virtual void PushCompressed(int key, const NDArray& comm_buf, const PSKV& pskv, int priority) {
    ...
    //std::cout<<"PushCompressed ZPull"<<std::endl;
  ...
  virtual void PushDefault(int key, const NDArray &send_buf, const PSKV& pskv, int priority) {
	  //std::cout<<"PushDefault ZPush"<<std::endl;
    auto push_to_servers =
...
//    std::cout<<"PushRowSparse ZPush"<<std::endl;
  ...
  virtual void PullDefault(int key, const NDArray &recv_buf, int priority) {
    auto pull_from_servers = [this, key, recv_buf](
      ...
      //std::cout<<"Node-1 PullDefault ZPull!"<<std::endl;
...
//	  std::cout<<"node-1 PullRowSparse_ ZPull"<<std::endl;
    using namespace rowsparse;
...
//	  std::cout<<"node-1 EncodeCompressedKey"<<std::endl;
    auto krs = ps::Postoffice::Get()->GetServerKeyRanges();
    const int num_servers = krs.size();
    CHECK_GT(num_servers, 0);
