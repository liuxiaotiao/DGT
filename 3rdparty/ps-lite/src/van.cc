  ...
  if (msg->meta.request) {
    ...
    bool Van::AsynAccept(Message* msg){
        ...
        if(msg->meta.seq == msg->meta.seq_end){
            ...
            /*********************************************************test*/
            /*if(msg->meta.sender == 9){
                std::cout << "push_op_num(AsynAccept) = " << msg->meta.push_op_num << ",key = " << msg->meta.first_key << ",cur_push rate =" << "("<< cur_push <<"/" << ct << ")"<< (float)cur_push/ct<<",complete rate = " << "("<< cn <<"/" << ct << ")"<<(float)cn/ct << ",delay avg iter = " << "("<< push_t <<"/" << push_n << ")"<<(float)push_t/push_n << std::endl;
            }*/
            /*********************************************************/
...
#ifdef DOUBLE_CHANNEL
      // start tcp receiver
//    std::cout<<"node-1 DOUBlE_CHANNEL"<<std::endl;
...
#else
    // start receiver
//    std::cout<<"node-1 is not DOUBLE_CHANNEL"<<std::endl;
    receiver_thread_ =
  ...
  start_mu_.unlock();
    std::cout <<my_node_.DebugString() << " start success !!"<<std::endl;
}
...
//		    std::cout<<"node-1 receving_tcp"<<std::endl;
                ProcessDataMsg(&msg);
...
//		    std::cout<<"node-1 receving_udp"<<std::endl;
                ProcessDataMsg(&msg);
                if(msg.meta.sender == 9)
                    udp_recv++;
...
//	    std::cout<<"node-1 receving"<<std::endl;
      ProcessDataMsg(&msg);
        if(msg.meta.sender == 9)
            tcp_recv++;//
    }
