    ...
    for(int i = 0; i < node.udp_port.size(); ++i){
        ...
        if (my_node_.id != Node::kEmpty) {
          ...
          if(zmq_setsockopt(udp_sender, ZMQ_TOS, &tos, sizeof(tos))==0){
             ...
             std::string command = "echo sudo -S iptables -t mangle -A OUTPUT -p udp --dst " + node.hostname + " --dport "+ std::to_string(node.udp_port[i])+" -j DSCP --set-dscp "+std::to_string(dscp);
