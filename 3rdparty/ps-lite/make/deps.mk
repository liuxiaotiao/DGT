...
URL3=https://github.com/liuxiaotiao
ifndef WGET
WGET = wget
endif
ifndef Git
...
${ZMQ}:
	$(eval FILE=zeromq-4.3.4.tar.gz)
	$(eval DIR=zeromq-4.3.4)
	$(eval NAME=zmq)
	rm -rf $(FILE) $(DIR) $(NAME)
	$(Git) $(URL3)/$(FILE) $(NAME)
	cd $(NAME) && tar --no-same-owner -zxvf $(FILE) && cd $(DIR) && export CFLAGS=-fPIC && export CXXFLAGS=-fPIC && ./configure --enable-drafts -prefix=$(DEPS_PATH) --with-libsodium=no --with-libgssapi_krb5=no && $(MAKE) && $(MAKE) install
	cd .. && rm -rf $(FILE) $(DIR)
	cd .. && rm -rf $(NAME)
